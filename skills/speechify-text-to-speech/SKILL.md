...
## Request fields
- `input` (required) — text or SSML, up to ~20,000 characters per request.
- `voice_id` (required) — a voice identifier from `voices.list()`.
- `model` (required) — e.g. a current `simba-*` model (verify; see `speechify-models`).
- `audio_format` (optional) — `mp3`, `wav`, `ogg`, `aac`.
...
Language is determined by the `model` + `voice_id`, not a top-level field — see
...
The response is JSON: base64-encoded `audio_data` plus `speech_marks` and billing
fields. Decode the base64 to write the audio file. (Confirm exact field names
against the live API reference.)
...
- Sending > ~20k characters in one request — chunk long text.
